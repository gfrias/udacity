...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  if (!is_initialized_) {
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      x_ = tools_.Polar2Cartesian(meas_package.raw_measurements_);
    }
...
void UKF::Prediction(double delta_t) {
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
    ...
    // state difference
    VectorXd x_diff = tools_.NormalizeVector(Xsig_pred_.col(i) - x);
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  NIS_laser_ = Update(meas_package, Zsig, R_laser_);
...
double UKF::Update(MeasurementPackage meas_package, MatrixXd Zsig, MatrixXd R) {
  int n_z = Zsig.rows();
  ...
  z_pred = tools_.NormalizeVector(z_pred);
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    // state difference
    VectorXd x_diff = tools_.NormalizeVector(Xsig_pred_.col(i) - x_);
  ...
  //residual
  VectorXd z = tools_.NormalizeVector(meas_package.raw_measurements_);
  
  VectorXd z_diff = tools_.NormalizeVector(z - z_pred);
